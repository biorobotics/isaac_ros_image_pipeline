#include "isaac_ros_image_proc/resize.hpp"
...
namespace nvidia
{
    namespace isaac_ros
    {
        namespace image_proc
        {
            ...
            Resize::Resize(const rclcpp::NodeOptions options)
                : Node("resize_node", options)
            {
                // Initialize params from generate parameter library
                param_listener_ = std::make_shared<resize_node::ParamListener>(this->get_node_parameters_interface());
                ...
                nitros_sub_ptr_ = std::make_shared<nvidia::isaac_ros::nitros::ManagedNitrosSubscriber<
                    ...
                    std::bind(&Resize::input_callback, this, std::placeholders::_1)); // diagnostics config and qos TODO
                ...
                interpolation_type_ = NVCV_INTERP_AREA; // See cvcuda/include/cvcuda/Types.h
            }

            void Resize::input_callback(const nvidia::isaac_ros::nitros::NitrosImageView &view)
            {
                ...
                const uint32_t output_image_width_ = params_.output_image_width_;
                const uint32_t output_image_height_ = params_.output_image_height_;
                ...
                nvcv::Tensor::Requirements output_image_reqs_ = nvcv::Tensor::CalcRequirements(
                    params_.batch_size_,
                    {static_cast<int32_t>(output_image_width_), static_cast<int32_t>(output_image_height_)},
                    nvcv::FMT_BGR8); // write a function to get the NVCV format from the image encoding TODO
                ...
                // resize processing.
                resize_op_(stream_, input_image_tensor_, output_image_tensor_, interpolation_type_);
...
RCLCPP_COMPONENTS_REGISTER_NODE(nvidia::isaac_ros::image_proc::Resize)