#include "isaac_ros_image_proc/morphology.hpp"
...
namespace nvidia
{
    namespace isaac_ros
    {
        ...
        {
            MorphologyNode::MorphologyNode(const rclcpp::NodeOptions &options)
                : Node("morphology_node", options)
            {
                // Initialize params from generate parameter library
                param_listener_ = std::make_shared<morphology_node::ParamListener>(this->get_node_parameters_interface());
                ...
                morph_type_ = NVCV_DILATE;
                border_type_ = NVCV_BORDER_CONSTANT;
                ...
                nitros_sub_ptr_ = std::make_shared<nvidia::isaac_ros::nitros::ManagedNitrosSubscriber<
                    ...
                    std::bind(&MorphologyNode::input_callback, this, std::placeholders::_1));
            ...
            void MorphologyNode::input_callback(const nvidia::isaac_ros::nitros::NitrosImageView &view)
            {
                ...
                nvcv::Size2D mask_size_(kernel_width_, kernel_height_);
                int2 anchor = {-1, -1}; // default anchor
                int32_t iteration = 1; // default iteration
                workspace_tensor_ = nvcv::Tensor(
                    batch_size_, {input_image_width_, input_image_height_},
                    nvcv::FMT_BGR8);
                morphology_op_(stream_, input_image_tensor_, output_image_tensor_,
                    workspace_tensor_, morph_type_, mask_size_, anchor, iteration, border_type_);
                RCLCPP_INFO(this->get_logger(), "Morphology processing...");
...
RCLCPP_COMPONENTS_REGISTER_NODE(nvidia::isaac_ros::image_proc::MorphologyNode)